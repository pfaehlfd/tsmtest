 ...
 *     Albert Flaig - data model refactoring, preparations for RAP
 *******************************************************************************/
package net.sourceforge.tsmtest.gui.breadcrumb;
...
public abstract class BreadcrumbViewer extends StructuredViewer {
	...
	for (final BreadcrumbItem item : items) {
	    if (element == item.element || element.equals(item.element)) {
		return item;
	    }
	}
	return null;
	...
	if (selectedItem == null || selectedItem.element == null) {
     ...
     * 
     * @see
     * org.eclipse.jface.viewers.Viewer#inputChanged(java.lang.Object,java.lang
     * .Object)
     */
    @Override
	...
	if (next) {
		...
		final ITreeContentProvider contentProvider = getContentProvider();
		if (!contentProvider.hasChildren(current.element)) {
		    return;
		}
