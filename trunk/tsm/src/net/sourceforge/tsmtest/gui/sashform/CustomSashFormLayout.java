...
/**
 * This class provides the layout for CustomSashForm
 * 
 * @see CustomSashForm
 */
public class CustomSashFormLayout extends Layout {
	...
	if (sashForm.sashes.length < controls.length - 1) {
	    CustomComposite[] newSashes = new CustomComposite[controls.length - 1];
	    System.arraycopy(sashForm.sashes, 0, newSashes, 0,
	...
	if (sashForm.sashes.length > controls.length - 1) {
		...
		sashForm.sashes = new CustomComposite[0];
	    } else {
		CustomComposite[] newSashes = new CustomComposite[controls.length - 1];
		System.arraycopy(sashForm.sashes, 0, newSashes, 0,
			newSashes.length);
	...
	if (sashForm.getOrientation() == SWT.HORIZONTAL) {
	    ...
	    if (controls.length > 1) {
		sashes[sashes.length - 1].setBounds(area.x, y, area.width,
			sashwidth);
		y += sashwidth;
		height = area.height - y;
		controls[controls.length - 1].setBounds(area.x, y, area.width,
			height);
	    }
